...
<div>
  ...
  this.renderProduct(product, count); ?? < Карточка товара 2 />
  <!-- результат вызова метода renderProduct -->
...
- Определить по элементу +/-, какой это товар (в верстке для этого есть `data-product-id` с уникальным идентификaтором товара)
...
Обновлять верстку нужно только в том случае, если модальное окно с корзиной открыто. Определить это можно по наличию класса `is-modal-open` на элементе `body`.
